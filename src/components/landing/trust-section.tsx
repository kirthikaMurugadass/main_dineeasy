...
export function TrustSection() {
  ...
  return (
    <section className="relative border-t border-border/60 bg-background py-16 font-sans sm:py-20 md:py-24 lg:py-28">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-10 2xl:max-w-[90rem]">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <FadeIn delay={0.06}>
            <h2
              className="mt-4 font-sans text-[clamp(1.8rem,2.8vw+0.5rem,2.3rem)] font-semibold leading-tight text-foreground"
          ...
          {metrics.map((metric: any) => (
            <FadeIn key={metric.label} delay={0.16}>
              <div className="rounded-2xl border border-border/70 bg-card/90 px-5 py-6 text-center shadow-soft">
                <p className="font-sans text-xl font-semibold text-foreground">{metric.value}</p>
                <p className="mt-2 font-sans text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
                  {metric.label}
                </p>
              </div>
            </FadeIn>
