...
export default function PaymentSuccessPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        if (!sessionId) {
          router.replace(qsRestaurant && qsMenuId ? `/public-menu/${encodeURIComponent(qsRestaurant)}/${encodeURIComponent(qsMenuId)}` : "/");
          return;
        }
        ...
        if (!confirmedOrderId) {
          toast.error(confirmData?.error || "Order details missing. Please place the order again.");
          router.replace(nextMenuHref);
          return;
        }
    ...
    return (
      <div className="relative flex min-h-screen items-center justify-center bg-background px-6 text-foreground">
        <canvas
          ...
          className={`pointer-events-none fixed inset-0 z-10 transition-opacity duration-200 ${confettiVisible ? "opacity-100" : "opacity-0"}`}
        />
